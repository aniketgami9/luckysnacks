...
function sendemail_verify($email,$otp){
    ...
    $mail->Username   = 'lucksnack64@gmail.com';                     //SMTP username
    $mail->Password   = 'lfndemnuuwcywwnr';  

    $mail->SMTPSecure = 'tls';            //Enable implicit TLS encryption
    $mail->Port       = 587; 

    $mail->setFrom('aniketgami64@gmail.com',"Lucky Snacks");
     ...
     && isset($_POST["email"]) && isset($_POST["confpass"]) && isset($_POST["name"]))
{
    ...
    if(empty($uname)){
        ...
        $sql00 = "SELECT * FROM users WHERE email='$email' ";
        ...
        if(mysqli_num_rows($result) > 0) {
            header("Location: signup.php?error=The User Name is taken already. Try another&$user_data");
            exit();
        }
        else if(mysqli_num_rows($result00)>1){ 
            $sql26 = "UPDATE `users` SET otp='$otp' WHERE email='$email'";
           ...
           $sql2 = "INSERT INTO users(username,password,name,email,otp) VALUES ('$uname', '$pass', '$name', '$email','$otp')";
